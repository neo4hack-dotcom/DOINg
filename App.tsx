...
const App: React.FC = () => {
  ...
  if (!appState) return <div className="flex h-screen items-center justify-center bg-slate-50 dark:bg-slate-900 text-slate-500">Loading DOINg...</div>;
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'management':
         if (appState.currentUser?.role !== UserRole.ADMIN) return <div className="p-8 text-red-500">Restricted Area. Admin Only.</div>;
  ...
  const getPageTitle = () => {
      switch(activeTab) {
          ...
          case 'weekly-report': return 'Weekly Status';
          case 'meetings': return 'Minutes & Action Items';
          case 'notes': return 'Notes & Canvas';
          case 'admin-users': return 'System Administration';
          case 'settings': return 'System Configuration';
          default: return 'DOINg Workspace';
      }
  }

  return (
    <div className="flex bg-slate-50 dark:bg-slate-950 min-h-screen font-sans transition-colors duration-200">
        ...
        <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 sticky top-0 z-40">
            ...
            <div className="flex items-center gap-6">
                ...
                <button 
                    ...
                    className="flex items-center gap-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-4 py-1.5 rounded-full text-sm font-bold shadow-md hover:shadow-lg transition-all transform hover:scale-105 active:scale-95"
                >
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button className="p-2 rounded-full text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors relative" title={reportNotification ? "Missing Weekly Report (>6 days)" : "Notifications"}>
                        <Bell className={`w-5 h-5 ${reportNotification ? 'text-red-500 animate-pulse' : ''}`} />
                        {reportNotification ? (
                            <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full border-2 border-white dark:border-slate-900"></span>
                        ) : (
                            <span className="absolute top-2 right-2 w-2 h-2 bg-green-500 rounded-full border-2 border-white dark:border-slate-900"></span>
                        )}
                    </button>
                </div>

                {/* User Profile */}
                <div className="flex items-center gap-3 pl-3 py-1.5 pr-1.5 rounded-full border border-transparent">
                    <div className="text-right hidden md:block">
                        <p className="text-sm font-semibold text-slate-900 dark:text-white">{appState.currentUser?.firstName}</p>
                        <p className="text-[10px] text-slate-500 uppercase font-bold">{appState.currentUser?.role}</p>
                    </div>
